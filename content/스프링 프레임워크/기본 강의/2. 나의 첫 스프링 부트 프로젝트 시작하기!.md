...
위 사진에서 demo란 폴더 보이시죠? 저 폴더처럼 애플리케이션 바로 상위에 있는 폴더명을 지정하게 됩니다.  
...
위에 사진을 보고 눈치채셨을 수 있지만 Name은 애플리케이션 이름을 변경해줍니다.  
위에 Artifact를 변경했을 때 Name이 shopping으로 같이 바뀌어서 DemoApplication.java가 ShoppingApplication.java로 변경된 것을 보실 수 있습니다.  
   
혹여나 Artifact와 다른 애플리케이션 이름을 사용하고 싶으시다면 Artifact를 지정한 다음에 Name을 변경해주시면 됩니다.  
   
...
우리는 Artifact와 같은 애플리케이션명을 사용할 것이기 때문에 다시 Name을 shopping으로 변경하겠습니다.
