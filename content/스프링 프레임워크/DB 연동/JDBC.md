...
> Java에서 데이터베이스와 연동을 위해서 먼저 커넥션 연결을 확인하고 SQL을 전달해 DB로부터 결과를 응답받습니다.<br><br>그러나 <u>데이터베이스의 종류(ex) MySQL, Oracle)에 따라 커넥션 연결, SQL, 결과 응답에 대한 방법이 다르기 때문</u>에 데이터베이스별로 코드를 각각 작성하는 대신, 자바에서 데이터베이스에 접속할 수 있도록 하는 자바 API인 JDBC가 등장합니다.
