...
![[Pasted image 20250212015802.png]]
...
![[Pasted image 20250212015837.png]]

---
# 📝 ERD 설계

![[Pasted image 20250212015856.png]]
...
### 비밀번호 처리
비밀번호는 Database 내에서도 암호화되어 저장해야 하기 때문에 `bcrypt` 모듈의 `hash()`함수를 사용하여 hash처리하고, 이를 Database에 저장하도록 하였습니다.
...
>`AuthGuard`를 사용하여 API 사용 시 JWT의 확인을 진행하려는데 요청 데이터에서 JWT 토큰을 확인하기 위해선 request 데이터를 봐야 하기 때문에 `AuthGuard`의 `getRequest()`를 실행하게 되는데, 여기서 request를 불러올 수 없는 오류가 발생했습니다
...
쪽지를 받은 사용자가 답장할 때에는 기존에 나에게 쪽지를 보낸 발신자가 수신자, 그리고 수신을 한 사용자가 발신자가 됩니다. 이를 이용하여 답장 쪽지에는 발신자와 수신자의 정보를 반대로 저장하고, 필요한 데이터와 카테고리는 기존에 수신한 쪽지와 동일하게 가져와서 생성하도록 구현했습니다.
```typescript
...
쪽지에 `is_read` 필드를 추가하여 해당 쪽지를 상대방이 읽었는지 안 읽었는지 확인할 수 있도록 하였습니다.
...
> AWS에서 EC2 서비스를 이용하여 인스턴스를 생성하였고, 상세 스펙은 아래와 같았습니다.
> `운영체제`: ubuntu
> `인스턴스 유형`: t2.small
> `지역`: ap-northeast-2
